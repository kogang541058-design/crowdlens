            ...
            <table class="reports-table">
                ...
                <tbody>
                    @if($reports->count() > 0)
                        ...
                        <tr class="report-row" data-disaster-type="{{ $report->disaster_type }}">
                            <td>#{{ $report->id }}</td>
                            <td>
                                ...
                                @if($report->status === 'pending')
                                    <div style="display: flex; gap: 0.5rem; align-items: center;">
                                        <form action="{{ route('admin.reports.verify', $report->id) }}" method="POST" style="display: inline;">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="status-btn status-pending">
                                                ⏳ Pending - Click to Verify
                                            </button>
                                        </form>
                                        <form action="{{ route('admin.reports.delete', $report->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('Mark this report as unverified? It will be removed from view.');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn-unverified">
                                                ❌ Unverified
                                            </button>
                                        </form>
                                    </div>
                            ...
                            <td>
                                @if($report->image)
                                    <a href="javascript:void(0)" onclick="showMedia('{{ Storage::url($report->image) }}', 'image')" class="view-link">View</a>
                                @else
                            ...
                            <td>
                                @if($report->video)
                                    <a href="javascript:void(0)" onclick="showMedia('{{ Storage::url($report->video) }}', 'video')" class="view-link">View</a>
                                @else
                                    <span style="color: #94a3b8;">N/A</span>
                                @endif
                            </td>
                            <td>
                                <div style="display: flex; gap: 0.5rem; align-items: center;">
                                    <form action="{{ route('admin.reports.markSolved', $report->id) }}" method="POST" style="display: inline;">
                                        @csrf
                                        @if($report->solved)
                                            <button type="submit" style="background: #d1fae5; color: #065f46; padding: 0.35rem 0.75rem; border-radius: 6px; font-size: 0.8rem; font-weight: 600; display: inline-block; cursor: pointer; border: none;">
                                                ✓ Solved
                                            </button>
                                        @else
                                            <button type="submit" style="background: #d1fae5; color: #065f46; padding: 0.35rem 0.75rem; border-radius: 6px; font-size: 0.8rem; font-weight: 600; display: inline-block; opacity: 0.5; cursor: pointer; border: none;">
                                                ✓ Solved
                                            </button>
                                        @endif
                                    </form>
                                    
                                    <form action="{{ route('admin.reports.markUnsolved', $report->id) }}" method="POST" style="display: inline;">
                                        @csrf
                                        @if($report->unsolved)
                                            <button type="submit" style="background: #fee2e2; color: #991b1b; padding: 0.35rem 0.75rem; border-radius: 6px; font-size: 0.8rem; font-weight: 600; display: inline-block; cursor: pointer; border: none;">
                                                ✗ Unsolved
                                            </button>
                                        @else
                                            <button type="submit" style="background: #fee2e2; color: #991b1b; padding: 0.35rem 0.75rem; border-radius: 6px; font-size: 0.8rem; font-weight: 600; display: inline-block; opacity: 0.5; cursor: pointer; border: none;">
                                                ✗ Unsolved
                                            </button>
                                        @endif
                                    </form>
