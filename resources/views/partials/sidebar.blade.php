<div class="w-full md:w-[260px] bg-gradient-to-b from-slate-800 to-slate-900 text-white md:fixed md:h-screen md:overflow-y-auto z-40 border-b md:border-b-0 border-slate-700 shadow-lg">
    ...
    <div class="p-4 md:p-6 border-b border-white/10 flex items-center justify-between md:block">
        <div>
            ...
            @auth('barangay')
                <h2 class="text-lg md:text-xl font-bold tracking-tight text-white truncate" title="{{ Auth::guard('barangay')->user()->username }}">
                    {{ Auth::guard('barangay')->user()->name }}
                </h2>
    ...
    <nav id="sidebarMenu" class="hidden md:block transition-all duration-300">
        <ul class="p-3 space-y-1">
            ...
            <li>
                <a href="{{ route('staff.settings') }}" class="group flex items-center py-2.5 rounded-lg text-sm transition-all {{ request()->routeIs('staff.settings') ? 'pl-3.5 pr-4 font-semibold bg-blue-600/20 text-white border-l-4 border-blue-500' : 'px-4 font-medium text-slate-300 hover:bg-white/10 hover:text-white' }}">
                    <svg class="w-5 h-5 mr-3 transition-colors {{ request()->routeIs('staff.settings') ? 'text-blue-400' : 'text-slate-400 group-hover:text-white' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
